...
var Token = function(value, expirationTimeInSeconds, scope) {
    ...
    if (!Cookies.get('suapToken')) {
      Cookies.set('suapToken', value, { expires: finishTime}, {sameSite: 'None'}, {secure: true});
    } else {
      value = Cookies.get('suapToken');
    }
  
    if (!Cookies.get('suapTokenExpirationTime')) {
      Cookies.set('suapTokenExpirationTime', finishTime, { expires: finishTime}, {sameSite: 'None'}, {secure: true});
    } else {
      finishTime = Cookies.get('suapTokenExpirationTime');
    }
  
    if (!Cookies.get('suapScope')) {
      Cookies.set('suapScope', scope, { expires: finishTime}, {sameSite: 'None'}, {secure: true});
    } else {
  ...
  console.log(this.getToken());
    ...
    this.getResource = function(scope, callback) {
      ...
      htmlReq.open('GET',resourceURL + '?scope=' + scope, true);
      htmlReq.setRequestHeader('Authorization', 'Bearer ' + token.getValue());
      htmlReq.setRequestHeader('Accept','application/json');
  
      htmlReq.onreadystatechange = function(){
        if(htmlReq.readyState == 4){
          let response;
          if(htmlReq.status == 200){
            response = JSON.parse(htmlReq.responseText);
            callback(response);
          }
          else{
    ...
    this.logout = function() {
      ...
      htmlReq.open('POST', logoutURL, true);
      htmlReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
      let data = 'token=' + encodeURIComponent(token.getValue()) + '&client_id=' + encodeURIComponent(clientID);
